<?php
/**
 * games/spelling_bee/spelling_bee_backend.php
 * YOPY Spelling Bee — Backend API
 */
session_start();
...
function handleStart(PDO $pdo): void {
    ...
    $gameId = GameService::getIdBySlug($pdo, 'spelling_bee');
    ...
    $_SESSION['behavior']['spelling_bee'] = [];
...
function handleScore(PDO $pdo): void {
    ...
    // Max: ~25 words × 20pts = 500; cap 1000
    ...
    try {
        ...
        respond($e->getCode() === '23000' ? 409 : 500, 'error',
                $e->getCode() === '23000' ? 'Duplicate (BR-05)' : 'DB error');
        return;
    }
    $_SESSION['current_game_state'] = 'RÉSULTAT_STOCKÉ';
    unset($_SESSION['behavior']['spelling_bee']);
