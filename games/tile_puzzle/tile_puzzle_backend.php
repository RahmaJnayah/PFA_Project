<?php
/**
 * games/tile_puzzle/tile_puzzle_backend.php
 * YOPY Tile Puzzle — Backend API
 */
session_start();
...
function handleStart(PDO $pdo): void {
    ...
    $gameId = GameService::getIdBySlug($pdo, 'tile_puzzle');
...
function handleScore(PDO $pdo): void {
    ...
    // Max: ~(1000 × 40/10) = 4000; cap 5000
    if ($points < 0 || $points > 5000) { respond(422,'error','Score out of range (BR-06)'); return; }
    ...
    unset($_SESSION['behavior']['tile_puzzle']);
